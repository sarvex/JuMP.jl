...
@register(model, f_foo_1, 2, foo_1)
@register(model, f_foo_2, 2, foo_2)
@objective(model, Max, f_foo_1(x[1], x[2]))
@constraint(model, f_foo_2(x[1], x[2]) <= 2)
function_calls = 0
optimize!(model)
...
set_silent(model)
@variable(model, x[1:2] >= 0, start = 0.1)
@register(model, f_foo_1, 2, memoized_foo[1])
@register(model, f_foo_2, 2, memoized_foo[2])
@objective(model, Max, f_foo_1(x[1], x[2]))
@constraint(model, f_foo_2(x[1], x[2]) <= 2)
function_calls = 0
optimize!(model)
